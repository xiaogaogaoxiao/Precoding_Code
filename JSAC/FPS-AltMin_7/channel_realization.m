function [ H,At,Ar,Fopt,Wopt ] = channel_realization(Nt,Nr,Ns,lamada)
     global Nc Nray;
%      power=sqrt(Nr*Nt/L);
...
%             AoA(2,[(c-1)*Nray+1:Nray*c]) = laprnd(1,Nray,AoA_m(2),angle_sigma);
            ...
            AoD(1,[(c-1)*Nray+1:Nray*c]) = laprnd(1,Nray,AoD_m,angle_sigma);
            
            AoA(1,[(c-1)*Nray+1:Nray*c]) = laprnd(1,Nray,AoA_m,angle_sigma);
...
%             Ar(:,j) = array_response(AoA(1,j),AoA(2,j),Nr);
            At(:,j) = array_response(AoD(1,j),Nt,d,lamada); %UPA array response
            Ar(:,j) = array_response(AoA(1,j),Nr,d,lamada);
