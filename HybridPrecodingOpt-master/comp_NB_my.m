...
clear all; close all; run_me_first;
%clc
%% set parameters
measure_snr_flag = true; realization = 100;
algorithms = { 'OMP', 'FPS-AltMin','PE-AltMin','Optimal', 'Proposed'};
alg_num = length(algorithms);
%Nt = 144;   % # of transmit antennas
%Nr = 36;    % # of receive antennas
Nt = 16*16; % # of transmit antennas 发射天线
Nr = 8*8;   % # of receive antennas 接收天线

Nc = 5;     % # of clusters 簇
Nray = 10;  % # of rays in each cluster 簇中的散射条
angle_sigma = 10/180*pi; % standard deviation of the angles in azimuth and elevation both of Rx and Tx

gamma = sqrt((Nt*Nr)/(Nc*Nray)); % normalization factor 归一化发射功率
sigma = 1; % according to the normalization condition of the H
if measure_snr_flag
    Ns = 4; 
    NRF_array = 6;   % # of RF channel
    %Ns = 8; 
    %NRF_array = 12;       
    SNR_dB = -15:5:10;
else
...
SNR = 10.^(SNR_dB./10); snr_num = length(SNR);   nrf_num = length(NRF_array);
%% set params
clear params;
params.Ns = Ns; params.Nc = Nc; params.Nray = Nray; 
params.Nt = Nt; params.Nr = Nr;
params.angle_sigma = angle_sigma;   params.gamma = gamma;     params.sigma = sigma;    
params.realization = realization;       

%% assign and initialize cells
alg_elapsed_time = cell(alg_num, nrf_num); info_cell        = cell(alg_num, realization);
speceff          = cell(alg_num, nrf_num); speceff_sd       = cell(alg_num, nrf_num);
for j=1:alg_num
    for k=1:nrf_num
        alg_elapsed_time{j,k} = 0;
    end
end
c = 1/sqrt(Nc)*exp(1i*[0:2*pi/Nc:2*pi-2*pi/Nc])'; % Nc = 5;% # of clusters
%% perform algorithms
for nrf_idx = 1:length(NRF_array)
    NRF = NRF_array(nrf_idx);    params.NRF = NRF;      % # of RF channel
    C = kron(eye(NRF),c);        
    fprintf('##################### NRF = %d ##################### \n', NRF);    
    ...
    for reali = 1:realization
        ...
        for alg_idx = 1 : alg_num
            alg_name = algorithms{alg_idx};            start_time = tic();
            switch alg_name
                case 'Optimal'
                    % Do nothing
                case 'MO-AltMin'
            ...
            for s = 1:snr_num
                ...
                end %sum(sum(abs(pinv(Wopt) -inv(Wopt'*Wopt)*Wopt')))
            end
