...
Nt = 144; Nr = 16; Ns = 4; NRF = 4;
SNR_dB = 0; SNR = 10.^(SNR_dB./10);
%realization = 10000;
realization = 110; 
Nc_candi = 5:5:50; CD = zeros(length(Nc_candi),1);
for i = 1:length(Nc_candi)
    ...
    for reali = 1:realization
        ...
        R(reali) = log2(det(eye(Ns) + SNR * pinv(WRF*WBB) * H * FRF * FBB * FBB' * FRF' * H' * WRF*WBB));  %(2) of [1]
    end
    CD(i) = mean(R);
end
